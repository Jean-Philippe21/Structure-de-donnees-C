...
struct retour *minMaxMoy(struct list_entier*L)
{   struct retour *p=NULL;
    p=malloc(sizeof(struct retour));
    if(p==NULL) exit(0);
    int som=0,i=0;
    struct list_entier*s=L;
  ...
  while(s->suivant!=NULL)
    ...
    if(s->val==L->val)
    {
            if((s->val)>=(s->suivant->val))
        {
            p->min=s->suivant->val;
        }
    }
        if((p->min)>=(s->suivant->val))
        {
            p->min=s->suivant->val;
        }
