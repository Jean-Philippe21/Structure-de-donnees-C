...
void insert(Dlist*p,int val,int val1)
{
     struct node *h=NULL;
     h=p->p_head;
  if(p->p_tail->data==val1)
   { Dlist *u=NULL;
     u=dlist_append(p,val);
     p=u;
   }

  else
   { struct node *u=NULL,*s=NULL;
     u=(struct node*)malloc(sizeof(struct node));
     if(u==NULL) {exit(0); }
     u->data=val;
     while(p->p_head->data!=val1)
     { s=p->p_head->p_next;
       p->p_head=s;
     }
     u->p_prev=p->p_head;
     u->p_next=p->p_head->p_next;
     p->p_head->p_next->p_prev=u;
     p->p_head->p_next=u;
     p->p_head=h;
     p->length++;
...
int main()
{
    int i,e=0,val=0,wi=0,val1=0,v=0,j=0;
    ...
    affiche(p_list);
     ...
     printf("\nEntrer 2 pour inserer un element dans la liste");
  scanf("%d",&v);
  if(v==2)
  { printf("\nApres quelle element voulez-vous inserer?\t");
    scanf("%d",&j);
    printf("\nEntrer la valeur de l'element a inserer:\t");
    scanf("%d",&val);
  ...
  affiche(p_list);
    ...
    printf("\nVoulez-vous supprimer un element? si oui entrer 1 si non entrer 0\t");
    scanf("%d",&wi);
    if(wi)
     {  printf("\nlequelle?\t");
        scanf("%d",&val1);
